...
class NodeRegister implements Serializable {
     ...
     */
    Node defineNode(Matrix matrix, boolean constantNode, int procedureID, int expressionID) throws MatrixException {
        if (entriesByMatrix.containsKey(matrix)) return nodeMap.get(matrix);
        else {
            Node node = new Node(matrix, constantNode);
    ...
    /**
     * Gets node map contained by register.
     *
    ...
    /**
     * Gets procedure ID corresponding the node.
     *
    ...
    /**
     * Gets expression ID corresponding the node.
     *
     * @param node node in question.
