...
public class GravesLSTMLayer extends AbstractExecutionLayer {
     ...
     */
    public GravesLSTMLayer(AbstractLayer parent, ActivationFunction activation, Init initialization, String params) throws DynamicParamException {
     ...
     * Executes Graves LSTM forward step equations, stores intermediate values to cache and writes layer output.<br>
    ...
    public void forwardProcess() throws MatrixException, NeuralNetworkException {
        ...
        for (Integer index : getOutsP().keySet()) {
            ...
            Matrix i = outPrev == null ? Wi.dot(in).add(bi) : Wi.dot(in).add(Ui.dot(outPrev)).add(Ci.multiply(cPrev)).add(bi);
            i.apply(i, sigmoid.getFunction());
            iS.put(index, i);

            // f = sigmoid(Wf * x + Uf * out(t-1) + Cf * c(t-1) + bf) → Forget gate
            Matrix f = outPrev == null ? Wf.dot(in).add(bf) : Wf.dot(in).add(Uf.dot(outPrev)).add(Cf.multiply(cPrev)).add(bf);
            ...
            Matrix o = outPrev == null ? Wo.dot(in).add(Co.multiply(c)).add(bo) : Wo.dot(in).add(Uo.dot(outPrev)).add(Co.multiply(c)).add(bo);
    ...
    /**
     * Takes single backward processing step for Graves LSTM layer to process sequence.<br>
    ...
    public void backwardProcess() throws MatrixException {
        ...
        TreeMap<Integer, Matrix> dCi = backward.getdWs(Ci);
        TreeMap<Integer, Matrix> dCf = backward.getdWs(Cf);
        TreeMap<Integer, Matrix> dCo = backward.getdWs(Co);
        ...
        for (Integer index : parent.getOuts().descendingKeySet()) {
            ...
            Matrix outC = index > 0 ? cS.get(index - 1) : null;

            dWo.put(index, dou.dot(outP));
            dCo.put(index, dc.multiply(cS.get(index)));
            dbo.put(index, dou);
            ...
            if (df != null) {
                ...
                dCf.put(index, dc.multiply(outC));
                dbf.put(index, df);
            ...
            if (index > 0) {
                ...
                dCi.put(index, di.multiply(outC));
